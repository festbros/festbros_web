(function($) {
    ...
    $(document).ready(function() {
        ...
        $('form.csi-subscribe-form').on('submit', function (evnt) {
            evnt.preventDefault();
            var $subform = $(this);
            var emailInput = $('form.csi-subscribe-form').find('input#email');
            var authenticity_token = $('form.csi-subscribe-form').find('input#authenticity_token').val();
            if (isEmail(emailInput.val())) {
                   
                $.ajax({
                    url: csi_path + '/Subscribe',
                    type: 'post',
                    data:  {'email': emailInput.val().toLowerCase() , 'authenticity_token' : authenticity_token},
                    beforeSubmit: function (argument) {
                    },
                    success: function (ajaxResponse) {
                        $('.csi-form-msg').addClass("alert alert-success csi-sub-alert").text(ajaxResponse.message);
        ...
        $contactForm.validate({
            submitHandler: function (form) {
                ...
                var authenticity_token = $contactForm.find('input#authenticity_token').val();
                var csiname = $contactForm.find('input#csiname').val();
                var csiemail = $contactForm.find('input#csiemail').val();
                var csisubject =$contactForm.find('input#csisubject').val();
                var csimessage = $contactForm.find('textarea#csimessage').val();
                 
                $.ajax({
                    url: csi_path + '/regform',
                    type: 'post',
